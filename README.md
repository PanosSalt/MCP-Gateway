...
3. Under **API permissions**, add:
   ...
   - **Application** (Microsoft Graph): `GroupMember.Read.All` (required for role sync during token refresh)
...
- Without `REDIS_URL` the limiter stores counters **in process memory**, so each
  ...
  enforcement.
- Behind a reverse proxy, set `TRUST_PROXY_HEADERS=true` so the key comes from
  ...
  proxy actually sets the header — clients can otherwise spoof it.
